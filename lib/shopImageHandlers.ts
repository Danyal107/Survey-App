...
export function shopImageBlobToken(): string | null {
  const t = process.env.BLOB_READ_WRITE_TOKEN?.trim();
  return t || null;
}

export async function putShopImageFromRequest(req: Request): Promise<NextResponse> {
  if (!shopImageBlobToken()) {
...
export async function deleteShopImageFromRequest(req: Request): Promise<NextResponse> {
  const token = shopImageBlobToken();
  if (!token) {
    return NextResponse.json(
